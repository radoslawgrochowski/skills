...
- Use `jj --no-pager` for commands that print output, for example `jj --no-pager log`, `jj --no-pager diff --git`, and `jj --no-pager show @`.
...
- Use `|` for revset unions, not commas: `jj --no-pager log -r "abc | def"`.
- Template fields are Jujutsu fields, not Git fields. Use `description`, not `desc`.
- If a command has surprising results, check `jj --no-pager op log`; recover the previous operation with `jj undo` when appropriate.
